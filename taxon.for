...
*   a) As a PROGRAM: the parameters are read from the file 'taxon.in',
...
*      as declared in taxon.par, not np.) 
*
...
*
*   3rd, 4th, 5th columns: morphology, number of fundamental 
*      frequencies, and number of resonances.
*
* code  dimension    morphol   nff  nres
*-----------------------------------------------------------------------
*
* 2D potentials
*
...
* 110   closed       box       1    1    aka closed boxlet
* 111   closed       loop      1    1
* 200   open         box       2    0
* 210   open         box       2    1    aka boxlet
* 211   open         loop      2    1
* 3xx   irregular    -        >2    -
*
* 3D potentials (the above 2D codes and their classifications also apply
* to 3D potentials if the orbits are linear or planar, aligned with the
* axes or the principal planes)
*
* 000   not classified         -    -
* 120   closed       box       1    2
...
* Output
      ...
      COMMON /results/ffund,code,lic1,lic2,licf,peri
      SAVE /results/
...
* Check whether the potential is correctly aligned
      CALL orientacion(ford(1,1),ford(2,1),ford(3,1),sw)
      IF(sw)GOTO 1

      nirr=1 ! Irregular orbit
      DO k=1,3
         IF(nl(k).NE.0)fmain(k)=ford(k,1)
      ENDDO
      IF(nl(1).GT.0)THEN
         cfm=1
...
* BEFORE the i.LE.nl(2)
         DOWHILE(i.LE.nl(2).AND.ltne(ford(2,MIN(i,nl(2))),ford(1,1)))
            i=i+1
         ENDDO
         IF(i.GT.nl(2))RETURN
         fmain(2)=ford(2,i)
      ...
      IF(nl(2).NE.0.AND.nl(3).NE.0)THEN
         i=1
         DOWHILE(i.LE.nl(3).AND.ltne(ford(3,MIN(i,nl(3))),ford(2,1)))
            i=i+1
         ENDDO
         IF(i.GT.nl(3))RETURN
         fmain(3)=ford(3,i)
      ...
      IF(nl(3).NE.0.AND.nl(1).NE.0)THEN
         i=1
         DOWHILE(i.LE.nl(3).AND.ltne(ford(3,MIN(i,nl(3))),ford(1,1)))
            i=i+1
         ENDDO
         IF(i.GT.nl(3))RETURN
         fmain(3)=ford(3,i)
...
************************************************************************
      SUBROUTINE orientacion(af1,af2,af3,sw)
      LOGICAL sw
      DOUBLE PRECISION af1,af2,af3
* Switches coordinates if the potential is not aligned with the x-y-z
* order (semimajor-intermediate-semiminor axes, respectively).
* This standard alignment should yield for the frequencies of the
* maximum amplitudes: fx < fy < fz.
************************************************************************
      INCLUDE 'taxon.par'
* Line spectra
      INTEGER nl(3)
...
c      af3=fmain(3)
      nls=nl(1)*nl(2)*nl(3)
      IF(jdim.EQ.2)THEN
         IF(ltne(af2,af1))THEN
            sw=.TRUE.
            IF(jpan.NE.0)WRITE(*,'(a/a)')
     &       ' *** Warning: freq(y) < freq(x)',
     &       ' *** Coordinates x-y will be switched'
            DO i=0,np-1
               aux=x(1,i)
               x(1,i)=x(2,i)
               x(2,i)=aux
            ...
            nl(2)=iaux
         ENDIF
      ELSEIF(jdim.EQ.3)THEN
         IF((nls.NE.0.AND.ltne(af2,af1).AND.ltne(af1,af3)).OR.
     &    (nl(3).EQ.0.AND.ltne(af2,af1)))THEN
            sw=.TRUE.
            IF(jpan.NE.0)WRITE(*,'(a/a)')
     &       ' *** Warning: freq(y) < freq(x)',
     &       ' *** Coordinates x-y will be switched'
            ...
            nl(1)=nl(2)
            nl(2)=iaux
         ELSEIF((nls.NE.0.AND.ltne(af3,af2).AND.ltne(af2,af1)).OR.
     &    (nl(2).EQ.0.AND.ltne(af3,af1)))THEN
            sw=.TRUE.
            IF(jpan.NE.0.AND.nl(2).EQ.0)THEN
               WRITE(*,'(a/a)')' *** Warning: freq(z) < freq(x)',
     &          ' *** Coordinates x-z will be switched'
            ELSEIF(jpan.NE.0)THEN
               WRITE(*,'(a/a)')
     &          ' *** Warning: freq(z) < freq(y) < freq(x)',
     &          ' *** Coordinates x-z will be switched'
            ENDIF
            DO i=0,np-1
               aux=x(1,i)
            ...
            nl(1)=nl(3)
            nl(3)=iaux
         ELSEIF((nls.NE.0.AND.ltne(af1,af3).AND.ltne(af3,af2)).OR.
     &    (nl(1).EQ.0.AND.ltne(af3,af2)))THEN
            sw=.TRUE.
            IF(jpan.NE.0)WRITE(*,'(a/a)')
     &       ' *** Warning: freq(z) < freq(y)',
     &       ' *** Coordinates y-z will be switched'
            DO i=0,np-1
               aux=x(2,i)
               x(2,i)=x(3,i)
               x(3,i)=aux
            ...
            nl(2)=nl(3)
            nl(3)=iaux
         ELSEIF(nls.NE.0.AND.ltne(af2,af3).AND.ltne(af3,af1))THEN
            sw=.TRUE.
            IF(jpan.NE.0)WRITE(*,'(a/a)')
     &       ' *** Warning: freq(y) < freq(z) < freq(x)',
     &       ' *** Coordinates x-y-z will be switched'
            DO i=0,np-1
               aux=x(1,i)
               x(1,i)=x(2,i)
               x(2,i)=x(3,i)
            ...
            nl(2)=nl(3)
            nl(3)=iaux
         ELSEIF(nls.NE.0.AND.ltne(af3,af1).AND.ltne(af1,af2))THEN
            sw=.TRUE.
            IF(jpan.NE.0)WRITE(*,'(a/a)')
     &       ' *** Warning: freq(z) < freq(x) < freq(y)',
     &       ' *** Coordinates x-y-z will be switched'
            DO i=0,np-1
               aux=x(1,i)
               x(1,i)=x(3,i)
...
* Line spectra
      ...
      DOUBLE PRECISION ampco,fm1,fm2,ampkj,ampkl,dpi
      SAVE prim,dpi
      DATA prim/.TRUE./
...
* Search for a line 2:1, 3:2, etc.
            DO j=1,nl(k)
               ...
               IF(metodo.EQ.1)THEN
                  ampkj=amp(k,j)
               ELSE
                  ampkj=amp(k,j)*ABS(1d0+dpi*fre(k,j))
               ENDIF
               IF(is0(fm1,fm2).AND.ampkj.GT.ampco)THEN ! candidate
                  ...
                  DO l=1,nl(k)
                     ...
                     IF(metodo.EQ.1)THEN
                        ampkl=amp(k,l)
                     ELSE
                        ampkl=amp(k,l)*ABS(1d0+dpi*fre(k,l))
                     ENDIF
* If there is a symmetric line with enough amplitude, then no subfamily
                     IF(is0(fm1,fm2).AND.ampkl.GT.ampco)GOTO 1
                  ENDDO
...
1              CONTINUE
      ...
      IF(metodo.EQ.1)THEN
         ampco=0.7d0*amp(k,1)
      ELSE
         ampco=0.8d0*amp(k,1)*ABS(1d0+dpi*fre(k,1))
      ENDIF
* Determine subfamily (heuristic approach): there is a subfamily if
            ...
            IF(euclides(i,m).EQ.1)THEN
               DO j=1,nl(k)
                  ...
                  ELSE
                     ampkj=amp(k,j)*ABS(1d0+dpi*fre(k,j))
                  ENDIF
                  IF(is0(fm1,-fm2).AND.ampkj.GT.ampco)THEN
                     ...
                     DO l=1,nl(k)
                        ...
                        ENDIF
* If there is a symmetric line with enough amplitude, then no subfamily
                        IF(ABS(fm1-fm2)/SQRT(fm1**2+fre(k,l)**2+
     &                   fre(k,j)**2).LE.eps.AND.ampkl.GT.ampco)GOTO 2
                     ENDDO
* There were no symmetric lines -> improper resonance
                     DO l=1,3
                        t1(l)=m*t1(l)
...
* Input orbit
      IF(jcom.EQ.0)THEN ! Read from file
         IF(jpan.NE.0)WRITE(*,*)'Reading input data...'
         CALL orbitin(jdim,arch)
      ENDIF
* Check np = 2**n
      dos=.FALSE.
      DO i=1,30
...
1        BACKSPACE(65)
         IF(jdim.EQ.3)THEN
            WRITE(65,'(8i6)')peri,code,(lic1(k),k=1,3),(lic2(k),k=1,3)
         ELSE
            WRITE(65,'(4i6)')peri,code,(lic1(k),k=1,2)
         ENDIF
         CLOSE(65)
      ENDIF
* Warnings about orbital periods
      IF(jpan.NE.0)THEN
         IF(peri.LE.80)WRITE(*,'(/a/)')
     &    '*** Warning: less than 80 periods'
         IF(peri.GT.300)WRITE(*,'(/a/)')
     &    '*** Warning: more than 300 periods'
      ENDIF
      END

************************************************************************
      SUBROUTINE orbitin(jdim,arch)
      INTEGER jdim
      CHARACTER arch*30
* Orbit input.
...
*    name already read by the input routine from the file "taxon.in".
* b) Times must be stored in the array DOUBLE PRECISION t(0:nn-1). Here 
*    "nn" is a parameter set in "taxon.par"; the actual number of points
...
* This is an example. "arch" is the string read from taxon.in.

      INTEGER k
* 2D orbit
      OPEN(51,FILE=arch,STATUS='OLD',ERR=2)
      DO i=0,nn-1
         IF(jdim.EQ.2)THEN
            READ(51,*,ERR=2)t(i),(x(k,i),k=1,2),(v(k,i),k=1,2)
         ELSE
            READ(51,*,ERR=2)t(i),(x(k,i),k=1,3),(v(k,i),k=1,3)
         ENDIF
      ENDDO
      CLOSE(51)
...
*-----------------------------------------------------------------------
      ...
      RETURN
2     STOP'ERROR: some error while opening/reading the orbit.'
      END
